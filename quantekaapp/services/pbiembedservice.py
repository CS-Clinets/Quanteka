...
from quantekaapp.services.aadservice import AadService
from quantekaapp.model.reportconfig import ReportConfig
from quantekaapp.model.embedtoken import EmbedToken
from quantekaapp.model.embedconfig import EmbedConfig
from quantekaapp.model.embedtokenrequestbody import EmbedTokenRequestBody
...
class PbiEmbedService:
    ...
    def get_embed_params_for_single_report(self, workspace_id, report_id, additional_dataset_id=None):
        ...
        if api_response.status_code != 200:
            HttpResponse({"description": f"Error while retrieving Embed URL\n{api_response.reason}:\t{api_response.text}\nRequestId:\t{api_response.headers.get('RequestId')}"}, status=api_response.status_code)
    ...
    def get_embed_token_for_single_report_single_workspace(self, report_id, dataset_ids, target_workspace_id=None):
        ...
        if api_response.status_code != 200:
            HttpResponse({"description": f"Error while retrieving Embed token\n{api_response.reason}:\t{api_response.text}\nRequestId:\t{api_response.headers.get('RequestId')}"}, status=api_response.status_code)
